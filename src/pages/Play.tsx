...
const SYSTEM_PROMPT = `You are PortfolioBot, an AI assistant embedded in NGUYEN MINH PHUONG creative portfolio website.
...
- Never fabricate employers, client names, project metrics, timelines, or tools.
- When asked about availability, pricing, or contracts: respond generally and invite the visitor to contact Phuong directly.
...
Owner: Nguyen Minh Phuong
Role: Marketing & Creative Director, Videographer, Content Creator
Location: Ho Chi Minh City, Vietnam
Contact: phuongnguyen.18042000@gmail.com | Phone: 0903601944
Education: Marketing, Hutech University (2018–2022)

Approach: Phuong loves beauty and translates it into every frame of his videos. He dislikes staying in the comfort zone and updates himself every day, turning ideas into emotional films that drive real results for brands.

Journey:
- 2020–2021: Marketing Social Media at John & Partners — content planning, poster design, TikTok ideation
- 2021–2022: Marketing Inhouse at Khui Beer Garden — marketing strategy, video editing, ads, remarketing
- 2022–2023: Marketing at FITMAR Agency — TikTok content, video ads, scripting, planning for clients and CEO
- 2023–2024: Creative — Marketing at Cong ty SW — personal branding ideas, ad management, viral clips
- 2024–Present: Video Editor & Content Creative at HUSSIO — editing and content research for the menswear brand

Current Role at HUSSIO:
- Editing ads, lookbooks and social content for HUSSIO smart-casual menswear
- Researching insights and trends, shaping content direction
- Concept, scripting and storyboarding for fashion campaigns
- Producing from pre-production to post, keeping brand voice consistent

Skills & Tools:
- Videography: concept, scripting, storyboard, shooting, editing, color grading, motion graphics, sound design
- Editing tools: Premiere Pro, After Effects, DaVinci Resolve, CapCut, Photoshop, Lightroom, Illustrator, Blender
- Marketing: content strategy, TikTok, Meta Ads, trend research, KOC/KOL, copywriting, personal branding, remarketing

Featured HUSSIO Campaigns:
1) Campaign Tet 2026 — concept, script, shooting, post for a modern-Tet brand film
2) So Mi Chong Nhan (Wrinkle-free Shirt) — product film highlighting fabric and benefits
3) HUAVIVAN x HUSSIO — collaboration campaign connecting two brands
4) So Mi Ba The He (Three Generations Shirt) — emotional brand film about lasting value
5) Soi Song — product film celebrating HUSSIO's signature fabric texture
6) Daily Fashion — short-form content series for styling HUSSIO in daily life

=== INTENT ROUTING ===
Classify visitor message and respond accordingly:

INTRO (about me, summary):
→ Give 2-4 sentence summary + top 2 campaigns + invite to contact

PROJECT (specific campaign question):
→ Concept → Role → Tools → Outcome

SKILLS (tools, expertise):
→ Grouped list (Videography / Editing / Marketing) + strongest areas

HIRING (availability, why hire):
→ Role fit + proof from campaigns + what can deliver + CTA to contact

CONTACT (how to reach):
→ Email, phone and social links, invite to use contact section
...
- If asked for sensitive info (ID, address), refuse and offer email contact instead

=== RESPONSE FOOTER ===
When appropriate, end with one of these:
- "Feel free to ask about any specific campaign!"
- "Want to discuss a collaboration? Use the contact section!"
- "Curious about Phuong's video or marketing work? Just ask!"`;

// Quick suggestion buttons for the chat
const QUICK_SUGGESTIONS = [
  "Cho tôi xem các campaign nổi bật",
  "Phương dùng công cụ gì?",
  "Phương có nhận dự án freelance không?"
];
...
const Play = () => {
  ...
  const [chatMessages, setChatMessages] = useState<ChatMessage[]>([
    { role: 'assistant', content: 'Xin chào! 👋 Chào mừng tới portfolio của Phương. Tôi có thể chia sẻ về các campaign HUSSIO, công cụ hoặc kinh nghiệm làm việc của Phương. Bạn muốn biết điều gì?' }
  ]);
  ...
  const updateGameStatus = useCallback((g: Chess) => {
    if (g.isCheckmate()) {
      setGameStatus(g.turn() === 'w' ? 'Chiếu hết! Quân đen thắng!' : 'Chiếu hết! Quân trắng thắng!');
    } else if (g.isDraw()) {
      if (g.isStalemate()) setGameStatus('Hòa cờ - hết nước đi');
      else if (g.isThreefoldRepetition()) setGameStatus('Hòa do lặp nước 3 lần');
      else if (g.isInsufficientMaterial()) setGameStatus('Hòa do không đủ quân');
      else setGameStatus('Hòa');
    } else if (g.isCheck()) {
      setGameStatus(g.turn() === 'w' ? 'Quân trắng đang bị chiếu!' : 'Quân đen đang bị chiếu!');
    } else {
      setGameStatus(g.turn() === 'w' ? "Lượt quân trắng" : "Lượt quân đen");
    }
  }, []);
  ...
  const flipBoard = () => {
    ...
    if (moveHistory.length > 0) {
      if (window.confirm('Bắt đầu ván mới?')) {
        resetGame();
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const errorMessage: ChatMessage = {
        role: 'assistant',
        content: 'Xin lỗi, có chút trục trặc kết nối. Bạn thử lại nhé? 😅'
      };
      ...
      <div className="chess-container">
        ...
        <div className="chat-panel">
          <div className="chat-header">
            <span className="chat-title"><FaComments style={{ color: '#fff' }} /> Trò chuyện cùng tôi</span>
          </div>
        ...
        <div className="chess-board-section">
          ...
          <div className="player-bar opponent-bar">
            <div className="player-info">
              <div className="player-avatar">
                <img src="/images/profile/avatar.png" alt="Nguyễn Minh Phương" />
              </div>
              <div className="player-details">
                <span className="player-name">Nguyễn Minh Phương</span>
                <span className="player-rating">{engineThinking ? '🤔 Đang suy nghĩ...' : 'ELO 3640'}</span>
              </div>
            </div>
          ...
          <div className="player-bar player-bar-bottom">
            <div className="player-info">
              ...
              <div className="player-details">
                <span className="player-name">Bạn</span>
                <span className="player-rating">{playerColor === 'w' ? 'Quân trắng' : 'Quân đen'}</span>
              </div>
            </div>
            <div className="captured-pieces">
        ...
        <div className="chess-side-panel right-panel">
          ...
          <div className="move-history">
            <div className="move-history-header">Nước đi</div>
